...
// http://api.jquery.com/?utm_content=buffer99509&utm_medium=social&utm_source=twitter.com&utm_campaign=buffer
...
var EightyApp = function() {
    this.processDocument = function(html, url, headers, status, jQuery) {
        ...
        // flag for content meta tags
        if(output.pubdate || output.author || output.keywords) {
            output.content_tags = true;
        }
        // add url path +length
        var match = url.match(/^(https?\:)\/\/(([^:\/?#]*)(?:\:([0-9]+))?)(\/[^?#]*)(\?[^#]*|)(#.*|)$/);
        if(match !== null) {
            output.path = match[5];
            output.path_length = output.path.length;
        }
        ...
        // page scrape runtime
        output.runtime_ms = Date.now() - start_time;
